...
The Arithmetic Reduction of Age family (Doyen & Gaudoin) is now implemented as `surpyval.recurrent.ARA`, parameterised by repair efficiency `rho` and memory `m`. Note the equivalences already covered elsewhere: ARA₁ = Kijima-I and ARA∞ = Kijima-II (both in `GeneralizedRenewal`), and the geometric process (Lam) = `GeneralizedOneRenewal` reparameterised by `a = 1/(1+q)`. Still to do: the Arithmetic Reduction of Intensity (ARI) family, which reduces the failure intensity rather than the virtual age and so needs the NHPP intensity framework rather than the lifetime-distribution machinery the ARA/Kijima models share.
