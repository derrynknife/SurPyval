...
**A flat likelihood.** Left censored observations combined with two or more distinct entry times can create a direction in which the likelihood rises without limit. A piece that one observation could have failed in, but that lies before another observation's entry time, is worth mass to the first and costs the second nothing: the second's likelihood is conditional on its own entry, so mass it never had a chance to see cancels from both the numerator and denominator. The likelihood then has no interior maximum and the EM keeps pushing mass towards the boundary instead of settling; raising ``max_iter`` does not help. SurPyval reports the share of the fitted mass in such pieces (more generally, pieces outside some observation's truncation window) as ``exploitable_mass`` and warns that the estimate is not identifiable when it exceeds 0.9. Entering every unit at a common time, or dropping the left censored observations, removes the problem.

The share is a screen, not a proof. Ordinary staggered-entry data has such pieces too and usually fits perfectly well, which is why a share is reported rather than a flag; but a well-identified fit can also pass 0.9, for example exact failures under staggered entry where the curve drops steeply at the first few failures, or exact data with right truncation (every piece above the smallest truncation time counts). A fit that warns but has ``converged`` True on data without left or interval censoring is usually such a case; with left truncation alone, compare it with ``KaplanMeier``.
